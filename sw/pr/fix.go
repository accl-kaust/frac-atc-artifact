...
func probeWorkload(conn net.Conn, timeout time.Duration, workloadID uint16, label string, gap time.Duration) error {
	...
	latency, err := sendOnly(conn, timeout, label, req)
	if err != nil {
		return err
	}
	fmt.Printf("%s: workload=0x%04x sent=%dB write_latency=%s\n", label, workloadID, len(req), latency)
	sleepGap(gap)
	return nil
}

func uploadBitstream(conn net.Conn, timeout time.Duration, hbmAddr uint64, prData []byte, chunkSize int, gap time.Duration) (int, error) {
	totalWritten := 0
	for offset := 0; offset < len(prData); {
		...
		req := buildReconfRequest(opWriteHBM, defaultSlotC00, addr, uint64(len(writePayload)), writePayload)
		writeLatency, readLatency, response, err := sendAndReadStatus(conn, timeout, fmt.Sprintf("WRITE_HBM[%d]", offset/chunkSize), req)
		if err != nil {
		...
		fmt.Printf("WRITE_HBM: offset=%d hbm_addr=0x%x payload=%dB request=%dB write_latency=%s status_read_latency=%s\n", offset, addr, len(writePayload), len(req), writeLatency, readLatency)
		totalWritten += len(writePayload)
		offset = end
		sleepGap(gap)
	}
...
func runReconfig(conn net.Conn, timeout time.Duration, hbmAddr uint64, prSize int, slotID byte, gap time.Duration) error {
	...
	latency, err := sendOnly(conn, timeout, "RECONF_ICAP", req)
	if err != nil {
		return err
	}
	fmt.Printf("RECONF_ICAP: slot=%d hbm_addr=0x%x size=%dB request=%dB write_latency=%s\n", slotID, hbmAddr, prSize, len(req), latency)
	sleepGap(gap)
	return nil
}
...
func queryStatus(conn net.Conn, timeout time.Duration, slotID byte, gap time.Duration) error {
	...
	latency, err := sendOnly(conn, timeout, "QUERY_ICAP_STATUS", req)
	if err != nil {
		return err
	}
	fmt.Printf("QUERY_ICAP_STATUS: request=%dB write_latency=%s\n", len(req), latency)
	sleepGap(gap)
	return nil
}
...
func main() {
	...
	chunkSize := flag.Int("chunk-size", 256, "maximum raw bitstream bytes per HBM write request")
	...
	sendGap := flag.Duration("send-gap", time.Millisecond, "delay after every request write")
	...
	postProbe := flag.Bool("post-probe", true, "send C00 workload probe after QUERY_ICAP_STATUS")
	flag.Parse()
	...
	written, err := uploadBitstream(conn, *timeout, *hbmAddr, prData, *chunkSize, *sendGap)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
